                ...
                if 'V3' == Parameters.ResToolsVersion:
                    ...
                    if dti_revertb0:
                        if nr_diffgrads != NumberOfVolumes:
                            ...
                            index = []
                            for i, e in enumerate(iSlices_sorted[:,13]):
                                if e == 1:
                                    index.append(i)
                        ...
                        if iSlices_sorted[-1,7] + 1 != Img_size:
                            b0moved = False
                        if b0moved:
                        ...
                        bval_filename = (os.path.join(pathstr, name)
                            + '-x-bval.txt')
                        try:
                            with open(bval_filename, 'wb') as bval_file:
                                bval_file.write(str(x)
                                    for x in bval[volumeRange,1])
                        except OSError:
                            logger.error('File name for bval.txt invalid')
                        #Write the bvec
                        bvec_filename = (os.path.join(pathstr, name)
                            + '-x-bvec.txt')
                        try:
                            with open(bvec_filename, 'wb') as bvec_file:
                                bvec_file.write(str(x)
                    ...
                    expnrslices = (Dim[2] * max(nr_mrtypes, nr_realmrtypes)
                        * nr_diffgrads * nr_dyn)
