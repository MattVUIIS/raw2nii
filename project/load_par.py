...
def load_par(filename):
    ...
    if not os.path.exists(filename):
        filename += 'v2'
    if not os.path.exists(filename):
        raise IOError('load_par.py: {0} does not exist'.format(filename))
    ...
    for image in par.img:
        for key, val in image['info'].items():
            q[key].extend(val)
    par.NumberOfVolumes = len(np.unique(q['dynamic_scan_num']))
    NoV = len(q['dynamic_scan_num'])//len(np.unique(q['slice_num']))
    if NoV != par.NumberOfVolumes:
        logger.warning('Dynamic Scan Number does not match number of slices. '
            'Assuming slices are ordered.')
        cnt = np.zeros((1, max(q['slice_num'])))
        for s in par.img:
            cnt[s['info']['slice_num']] += 1
            s['info']['dynamic_scan_num'] = cnt[s['info']['slice_num']]
    ...
    if not np.allclose(scn['slicegap'], 0):
        ...
        if is_in_volume_order:
            ...
            if is_in_slice_order:
                ...
                logger.warning('load_par.py: Slice ordering is not a '
                    'predefined type.')
...
def parseScanImgLine(par, line, imgTagFormat):
    ...
    if 'pix_bits' not in par['scn']:
        ...
        if not np.allclose(par['scn']['pix_bits'], img['info']['pix_bits']):
            logger.warning('load_par.py - REC file contains image slices with '
                'variable bits per pixel.')
